...
class Robot:
	def __init__(self, args):
		...
		self.right_sleep_time = 3.5
		self.left_move_in_time = 0.4
		self.straight_move_in_time = 0.4
		self.right_move_in_time = 0.15
		...
		self.turn_system = TurnSystem(self.left_angle, self.straight_angle, self.right_angle, self.left_throttle, 
			...
			self.left_move_in_time, self.straight_move_in_time, self.right_move_in_time)
		...
		while(True):
			try:
				...
				while(True):
					...
					#red_mask = cv2.inRange(hsv_frame, (1, 100, 0), (20, 255, 255))
					#red = np.bitwise_and(red, red_mask)
					#blue = np.zeros_like(blue)
					#green = np.zeros_like(green)
					#temp_frame = cv2.merge([blue, green, red])
					#cv2.imshow("red frame", temp_frame)
				...
				steering, throttle, sleep_time, move_in_time = self.turn_system.input_turn()
